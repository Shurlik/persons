...
import { Box, Drawer, List, ListItem, Collapse, Typography } from '@mui/material';
...
const Sidebar = () => {
  ...
  return (
    ...
    >
      ...
      >
        <StrategySection />
        <FunnelSection openItems={openItems} handleClick={handleClick} />
        <SalesSection />
        <ContentMarketingSection />
        <BrandSection openItems={openItems} handleClick={handleClick} />
      </Box>
    </Drawer>
  );
};
...
const BrandSection = ({ openItems, handleClick }) => {
  ...
  return (
    <Box sx={{ paddingBottom: '120px' }}>
      <Typography sx={{ color: colors.white, marginBottom: '8px', fontWeight: 'bold', borderTop: `1px solid ${colors.darkGrayMain}`, paddingTop: '16px' }}>
        Brand
      </Typography>
      {brandItems.map((item) => (
        <React.Fragment key={item.name}>
          ...
          <Collapse in={openItems[item.name]} timeout="auto" unmountOnExit>
            <List component="div" disablePadding sx={{ padding: '8px 12px', backgroundColor: colors.greybg, borderRadius: '4px' }}>
              {item.subItems.map((subItem) => (
                <ListItem button key={subItem.name} sx={{ borderRadius: '4px', '&:hover': { backgroundColor: colors.greyhover, color: colors.orange } }}>
                  {subItem.icon}
                  <Link to="/" style={{ textDecoration: 'none', marginLeft: '8px', color: 'inherit' }}>
        ...
        item.subItems ? (
          <React.Fragment key={item.name}>
            ...
            >
              <Box sx={{ display: 'flex', alignItems: 'center', color: 'inherit' }}>
                {item.icon}
                <Typography sx={{ fontWeight: '700', marginLeft: '8px', fontSize: '14px', color: 'inherit' }}>{item.name}</Typography>
              </Box>
              {openItems[item.name] ? <ExpandLess sx={{ color: 'inherit' }} /> : <ExpandMore sx={{ color: 'inherit' }} />}
            </Box>
            <Collapse in={openItems[item.name]} timeout="auto" unmountOnExit>
          ...
          <ListItem button key={item.name} sx={{ borderRadius: '4px', '&:hover': { backgroundColor: colors.greyhover, color: colors.orange } }}>
            {item.icon}
            <Link to={item.link} style={{ textDecoration: 'none', marginLeft: '8px', color: 'inherit' }}>
